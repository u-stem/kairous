...
test-small ────────┤
migration ─────────┤
build (new) ───────┼──> test-large (artifact download + bun run start)
                   │
                   └──> lighthouse (artifact download + bun run start)
test-medium (既存、Supabase 起動)
```

`build` は lint/typecheck と並列開始可能。`test-large` と `lighthouse` は `build` 完了後に fan-out (並列実行)。

当初計画では test-large を `--shard=i/4` matrix で並列化予定だったが、Playwright project 依存により効果が得られないことが判明し revert。詳細は Non-Goals を参照。
...
build artifact 消費 + `CI_REUSE_SERVER=1` による外部サーバ再利用:
...
`tests/large/playwright.config.ts` の `webServer.command` に `CI_REUSE_SERVER=1` で `bun run build && bun run start` ではなく `bun run start` のみ実行する分岐を追加。外部で起動済みの prod サーバを再利用させる。
