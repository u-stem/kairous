...
Claude PR Review は **CI (lint / typecheck / test:small / test:medium / test:large / lighthouse / build) が全 success した時点で自動実行される** (`workflow_run` トリガー)。CI が failure / cancelled の場合は無意味なレビューとトークン浪費を避けるため起動しない。
...
- **CI が赤のときは Claude Review が走らない**: CI 失敗は先に直す。CI を緑にすると数分後に Claude Review が自動起動する
