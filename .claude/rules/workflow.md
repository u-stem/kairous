...
- PR 作成後に `gh pr merge <N> --auto --squash` で auto-merge を enable。branch protection の required status checks (現行は `gh api repos/u-stem/kairous/branches/main/protection --jq .required_status_checks.contexts` で確認) が全 pass すると自動で squash merge される
