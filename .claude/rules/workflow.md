...
推奨は `bun run worktree:create` / `worktree:cleanup` 経由 (#265)。ディレクトリ命名 (`../kairous-<issue>-<slug>`) と後始末 (remove + branch -d + prune) を 1 コマンドに束ねる。`worktree:create` は `supabase/migrations/` の最大番号 + 他 worktree の予約番号を避けた次番号を予約して標準出力に表示し、`.kairous/worktree.json` に記録する (#266)。並列 DB スキーマ変更時の番号衝突を事前回避できる。
...
- `.github/workflows/*.yml` を触る PR が main に merge された直後、先行する feat branch は default branch と workflow 不一致になり Claude PR Review が 401 Workflow validation failed になる。`./scripts/rebase-open-prs.sh --run` で自分宛 open PR の branch を一括 main 同期 + push できる。conflict 発生 branch のみ手動解決
