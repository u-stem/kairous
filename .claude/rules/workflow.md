...
- **workflow 自体を変更する PR**: `.github/workflows/*.yml` を変更する PR では、claude-code-action の保護機構により「workflow file must exist and have identical content to the version on the repository's default branch」として 401 Unauthorized となり Claude PR Review が走らない。ローカル code-reviewer エージェントでの事前レビューで代替し、マージ後の次 PR で挙動を検証する (例: #212)
